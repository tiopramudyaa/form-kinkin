...
<html lang="id">
    ...
    <body
        ...
        style="
            ...
            {{ $palette['cute'] ? "font-family: 'Fredoka', ui-sans-serif, system-ui, sans-serif;" : '' }}
        "
    >
        @if (($progress ?? 0) > 0)
        ...
        <div class="relative z-10 flex min-h-screen items-center justify-center px-4 py-12">
            <div class="w-full max-w-xl">
                <div
                    class="bg-white/90 p-6 shadow-sm backdrop-blur sm:p-10"
                    style="border-radius: var(--radius); border: 1px solid color-mix(in srgb, var(--accent) 15%, white);"
                >
                    @yield('content')
                </div>
            </div>
