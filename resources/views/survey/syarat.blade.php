...
@section('content')
    ...
    <form method="POST" action="{{ route('survey.syarat.store') }}" class="mt-8">
        @csrf

        <label class="option-card flex items-start gap-3 text-sm">
            <input type="checkbox" name="agree" value="1" class="mt-0.5" required>
            <span>Saya telah membaca dan menyetujui syarat & ketentuan di atas.</span>
        </label>

        @error('agree')
            <p class="mt-2 text-xs text-red-600">Kamu harus menyetujui syarat & ketentuan dulu ya.</p>
        @enderror

        <button type="submit" class="btn-primary mt-6">Lanjut</button>
    </form>
@endsection
