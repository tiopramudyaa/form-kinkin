...
@section('content')
    ...
    <p class="mt-2 text-sm text-gray-600">Isi data diri kamu dengan benar ya.</p>
    ...
    <form method="POST" action="{{ route('survey.data-diri.store') }}" class="mt-6 space-y-4">
        ...
        <div>
            <label for="birth_date" class="text-sm font-medium">Tanggal Lahir</label>
            <input type="date" name="birth_date" id="birth_date" value="{{ old('birth_date') }}" required class="form-input">
        </div>

        <div>
            <label for="nickname" class="text-sm font-medium">Nama Panggilan</label>
